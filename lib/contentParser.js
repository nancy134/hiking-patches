...
export const getSinglePage = (folder, filters) => {
  ...
  var publishedPages = singlePages.filter(
    (page) =>
      !page.frontmatter.draft && page.frontmatter.layout !== "404" && page
  );

  if (filters && filters.state){
    publishedPages = publishedPages.filter(
      (page) =>
        page.frontmatter.state && page.frontmatter.state == filters.state && page
    );
  }
  ...
  if (filters && filters.states){
    publishedPages = publishedPages.filter(
      (page) =>
        page.frontmatter.states && page.frontmatter.states.includes(filters.states) && page
    );
  }
